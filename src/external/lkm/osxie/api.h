...
#define OSXIE_BSD_ARG(_type, _name) _type _name; char _ ## _name ## _padding[(sizeof(uint64_t) <= sizeof(_type) ? 0 : sizeof(uint64_t) - sizeof(_type))]

#pragma pack (push, 1)

enum { NR_get_api_version = OSXIE_MACH_API_BASE,
	NR_mach_reply_port,
	...
	OSXIE_MACH_API_ALMOST_COUNT, // don't use this directly (it's offset by the API base number); use `OSXIE_MACH_API_COUNT`
};

#define OSXIE_MACH_API_COUNT (OSXIE_MACH_API_ALMOST_COUNT - OSXIE_MACH_API_BASE)
...
{
	OSXIE_BSD_ARG(uint64_t, stackaddr);
	OSXIE_BSD_ARG(uint64_t, freesize);
	OSXIE_BSD_ARG(uint32_t, port);
	OSXIE_BSD_ARG(uint32_t, sem);
};

struct psynch_mutexwait_args
{
	OSXIE_BSD_ARG(uint64_t, mutex);
	OSXIE_BSD_ARG(uint32_t, mgen);
	OSXIE_BSD_ARG(uint32_t, ugen);
	OSXIE_BSD_ARG(uint64_t, tid);
	OSXIE_BSD_ARG(uint32_t, flags);
};
...
{
	OSXIE_BSD_ARG(uint64_t, cv);
	OSXIE_BSD_ARG(uint64_t, cvlsgen);
	OSXIE_BSD_ARG(uint32_t, cvugen);
	OSXIE_BSD_ARG(uint64_t, mutex);
	OSXIE_BSD_ARG(uint64_t, mugen);
	OSXIE_BSD_ARG(uint32_t, flags);
	OSXIE_BSD_ARG(int64_t, sec);
	OSXIE_BSD_ARG(uint32_t, nsec);
};
...
struct psynch_cvbroad_args
{
	OSXIE_BSD_ARG(uint64_t, cv);
	OSXIE_BSD_ARG(uint64_t, cvlsgen);
	OSXIE_BSD_ARG(uint64_t, cvudgen);
	OSXIE_BSD_ARG(uint32_t, flags);
	OSXIE_BSD_ARG(uint64_t, mutex);
	OSXIE_BSD_ARG(uint64_t, mugen);
	OSXIE_BSD_ARG(uint64_t, tid);
...
{
	OSXIE_BSD_ARG(uint64_t, cv);
	OSXIE_BSD_ARG(uint32_t, cvgen);
	OSXIE_BSD_ARG(uint32_t, cvugen);
	OSXIE_BSD_ARG(uint32_t, cvsgen);
	OSXIE_BSD_ARG(uint32_t, prepocnt);
	OSXIE_BSD_ARG(uint32_t, preposeq);
	OSXIE_BSD_ARG(uint32_t, flags);
};
...
{
	OSXIE_BSD_ARG(uint64_t, rwlock);
	OSXIE_BSD_ARG(uint32_t, lgenval);
	OSXIE_BSD_ARG(uint32_t, ugenval);
	OSXIE_BSD_ARG(uint32_t, rw_wc);
	OSXIE_BSD_ARG(int, flags);
};
...
struct kevent_args {
	OSXIE_BSD_ARG(int, fd);
	OSXIE_BSD_ARG(uint64_t, changelist);
	OSXIE_BSD_ARG(int, nchanges);
	OSXIE_BSD_ARG(uint64_t, eventlist);
	OSXIE_BSD_ARG(int, nevents);
	OSXIE_BSD_ARG(uint64_t, timeout);
};
...
struct kevent64_args {
	...
	OSXIE_BSD_ARG(uint64_t, eventlist);
	OSXIE_BSD_ARG(int, nevents);
	OSXIE_BSD_ARG(unsigned int, flags);
	OSXIE_BSD_ARG(uint64_t, timeout);
};
...
struct kevent_qos_args {
	...
	OSXIE_BSD_ARG(uint64_t, eventlist);
	OSXIE_BSD_ARG(int, nevents);
	OSXIE_BSD_ARG(uint64_t, data_out);
	OSXIE_BSD_ARG(uint64_t, data_available);
	OSXIE_BSD_ARG(unsigned int, flags);
};
