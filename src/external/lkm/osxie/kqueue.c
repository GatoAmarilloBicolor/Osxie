...
// our own functions are prefixed with `dkq` or `dkqueue` (Osxie kqueue) to avoid possible conflicts
...
static void dkqueue_proc_listener(int pid, void* context, osxie_proc_event_t event, unsigned long extra) {
	...
	*saved_data = 0;
	...
	dkqueue_log("received event %d from Osxie's process notification system for Linux PID %d with extra=%ld", event, pid, extra);
...
static int dkqueue_proc_attach(struct knote* kn, struct kevent_qos_s* kev) {
	...
	//       do we want to implement them in Osxie for backwards compat?
	...
	if (!linux_pid) {
		...
		dkqueue_log("failed to find XNU task for Osxie PID %ld", kn->kn_id);
		goto error_out;
	}
	xnu_pid = pid_nr(linux_pid);
	rcu_read_unlock();

	dkqueue_log("attaching process filter to process with Osxie PID %ld and Linux PID %d", kn->kn_id, xnu_pid);
...
static void dkqueue_vnode_update_wanted_events(dkqueue_vnode_context_t* ctx) {
	...
	VNODE_TRANSLATE(NOTE_FUNLOCK, 0); // not sure how we're supposed to detect this (Osxie userspace hook and hope that Linux doesn't touch the file?)
...
int osxie_kqueue_create(struct task* task) {
	...
	if (!proc) {
		...
		// since it's currently impossible to create a plain Mach task in Osxie
		result = -LINUX_EINVAL;
