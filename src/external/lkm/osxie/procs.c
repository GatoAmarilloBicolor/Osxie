 ...
 *
 * note: these functions are entierly Osxie-specific; the XNU code actually does all proc-related creation and destruction
 * directly in `proc` management functions like `proc_exit` (indirectly, through `reap_child_locked`) and `forkproc` (directly)
 *
 * we don't really need to manage `proc`s in Osxie (at least not right now), so we just need to allocate and free them
 */
 ...
 *
 * Osxie's `proc` PIDs are taken from their corresponding Linux processes. However, this counter is BSD-specific, and doesn't need to be
...
proc_t darling_proc_create(task_t task) {
	...
	// grab our PID
	// XNU code keeps a separate BSD counter, but our `task_pid` returns our namespaced Linux PID, which is what everyone in Osxie cares about
