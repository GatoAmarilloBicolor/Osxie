...
thread_t osxie_thread_get(unsigned int tid);

void osxie_task_register(task_t t);
 ...
 * @param context Additional data passed to `osxie_proc_notify_register` when registering this listener
 * @param event The event that occurred on the process
 * @param extra Additional event-dependent data
 */
typedef void (*osxie_proc_notification_listener)(int pid, void* context, osxie_proc_event_t event, unsigned long int extra);
 ...
 * @param registration_id The registration ID of the listener, returned by `osxie_proc_notify_register`
...
void osxie_thread_register(thread_t t);
...
_Bool osxie_task_key_set(unsigned int key, void* value, task_key_dtor dtor);
