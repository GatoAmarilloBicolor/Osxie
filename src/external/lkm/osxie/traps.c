...
#define TRAP(num, impl) [num - OSXIE_MACH_API_BASE] = { (trap_handler) impl, #impl }

static const struct trap_entry mach_traps[OSXIE_MACH_API_COUNT] = {
	// GENERIC
...
static int mach_init(void)
{
	...
	printk(KERN_INFO "Osxie Mach: kernel emulation loaded, API version " OSXIE_MACH_API_VERSION_STR "\n");
	return 0;

fail:
	printk(KERN_WARNING "Error loading Osxie Mach: %d\n", err);
...
long mach_dev_ioctl(struct file* file, unsigned int ioctl_num, unsigned long ioctl_paramv)
{
	...
	ioctl_num -= OSXIE_MACH_API_BASE;

	if (ioctl_num >= OSXIE_MACH_API_COUNT)
	{
		rv = -LINUX_ENOSYS;
		goto out;
	}
...
int kernel_printk_entry(task_t task, struct kernel_printk_args* in_args)
{
	...
	printk(KERN_DEBUG "Osxie TID %d (PID %d) says: %s\n", current->pid, current->tgid, args.buf);
