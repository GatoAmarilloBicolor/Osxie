...
#include <osxieserver/duct-tape/psynch.h>
...
void thread_hold(thread_t xthread) {
	...
	if (xthread->suspend_count++ == 0) {
		// This signal leads to sigexc.c which will end up calling osxieserver;
		// osxieserver will hold the caller so long as the suspend_count > 0.
