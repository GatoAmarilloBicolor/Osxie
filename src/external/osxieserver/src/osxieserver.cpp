...
#include <osxieserver/server.hpp>
#include <osxieserver/config.hpp>

#ifndef OSXIESERVER_INIT_PROCESS
	#define OSXIESERVER_INIT_PROCESS "/sbin/launchd"
#endif

#ifndef OSXIESERVER_XDG_USER_DIR_CMD
	#define OSXIESERVER_XDG_USER_DIR_CMD "xdg-user-dir"
#endif
...
const char* xdgDirectory(const char* name)
{
	...
	char* cmd = (char*) malloc(sizeof(OSXIESERVER_XDG_USER_DIR_CMD) + 1 + strlen(name));

	sprintf(cmd, OSXIESERVER_XDG_USER_DIR_CMD " %s", name);
...
void spawnLaunchd(const char* prefix)
{
	...
	auto tmp = (std::string(prefix) + "/.osxieserver.sock");

	const char* initPath = getenv("DSERVER_INIT");

	if (!initPath) {
		initPath = OSXIESERVER_INIT_PROCESS;
	}
...
static bool shouldUseOverlayFs() {
	...
	if (testEnvVar("OSXIE_NOOVERLAYFS")) {
		shouldUse = false;
		explicitlySet = true;
	} else if (getenv("OSXIE_NOOVERLAYFS")) {
		shouldUse = true;
		explicitlySet = true;
	}
...
int main(int argc, char** argv) {
	...
	if (argc < 6) {
		fprintf(stderr, "osxieserver is not meant to be started manually\n");
		exit(1);
	}
	...
	if (getuid() != 0 || getgid() != 0) {
		fprintf(stderr, "osxieserver needs to start as root\n");
		exit(1);
	}
