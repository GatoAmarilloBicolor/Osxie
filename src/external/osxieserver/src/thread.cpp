	...
	_process(process)
{
	...
	if (_tid == -1) {
		throw std::system_error(ESRCH, std::generic_category(), "Failed to find thread ID within osxieserver's namespace");
	}
...
/*
 * server-to-client (S2C) calls are used by osxieserver to invoke certain functions within managed processes
