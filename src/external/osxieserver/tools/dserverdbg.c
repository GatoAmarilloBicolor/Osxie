...
static char* default_prefix_path(uid_t original_uid) {
	...
	if (asprintf(&result, "%s/.osxie", info->pw_dir) < 0) {
		return NULL;
	}
...
int main(int argc, char** argv) {
	...
	snprintf(__dserver_socket_address_data.sun_path, sizeof(__dserver_socket_address_data.sun_path), "%s/.osxieserver.sock", prefix_path);
	...
	if (__dserver_main_thread_socket_fd < 0) {
		fprintf(stderr, "Failed to set up osxieserver client socket\n");
		return 1;
	}
