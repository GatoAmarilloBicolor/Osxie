...
int main(int argc, char** argv) {
	...
	if (snprintf(default_output_name, sizeof(default_output_name), "%s/osxiecore-%s", dirname(tmp_output_dirname), basename(tmp_output_basename)) < 0) {
		perror("snprintf");
		return 1;
	}
	...
	if (snprintf(default_prefix, sizeof(default_prefix), "%s/.osxie", homedir) < 0) {
		perror("snprintf");
		return 1;
	}
